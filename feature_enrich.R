...
#(manip raw) user frequency (i.e. how familiar w/ the app), rider frequency (i.e. how experienced is rider)
...
worker <- group_by(df, rider_id, as.Date(read_tm)) %>%
  arrange(read_tm) %>%
  mutate(new_shift=ifelse(is.na(lag(finish_tm)),FALSE,((as.numeric(read_tm)-as.numeric(lag(finish_tm)))/3600)>1), shift=cumsum(new_shift))
worker <- group_by(worker) %>% group_by(rider_id, as.Date(read_tm), shift) %>% 
...
dfl_user <- group_by(dfl, user_id) %>%
  arrange(user_id, finish_tm) %>%
  summarize(n=n(), delay_avg=mean(delay), dbad_avg=mean(day_bad), tmax_avg = mean(temp_max),
            rinc_avg=mean(rider_income), lunch=as.factor(mean(hour(finish_tm))>10 & mean(hour(finish_tm))<14), prereq_avg = mean(require_tm - place_tm), 
            price_avg=mean(price), price_sd=sd(price), span=max(finish_tm)-min(finish_tm))
dfl <- left_join(dfl, dfl_user, by="user_id")
